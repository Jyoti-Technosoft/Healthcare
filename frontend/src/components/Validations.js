...
export function validatePatternPassword(password) {
  ...
  return passwordPattern.test(password)
    ? ""
    : "Password must contain at least six characters, including numbers, letters, and symbols!";
}
...
export const calculateIdealWeight = (height) => {
  ...
  const minWeight = Math.round(exactWeight * 0.9);
  const maxWeight = Math.round(exactWeight * 1.1);
...
export function validateHeightRange(height) {
  if (!height || height.trim() === "") return "Height is required";
  const value = parseFloat(height);
  if (isNaN(value)) return "Enter valid height";
  if (value < 50 || value > 300) return "Height must be between 50 cm and 300 cm";
  return "";
}
...
export function validateWeightRange(weight) {
  ...
  if (isNaN(value)) return "Enter valid weight";
  if (value < 2 || value > 500) return "Weight must be between 2 kg and 500 kg";
  return "";
}
